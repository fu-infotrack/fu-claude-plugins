...
2. Pull a representative sample event for the **stack frames** (pup's `get` gives file/function but not a full multiline stack): `pup traces search --query 'service:<service> @issue.id:<issue_id>' --from "<first_seen-ish>" --limit 3` and read `@error.stack` / span error attributes. If that returns no spans (issue is log-sourced, or spans aged out), fall back to `pup logs search --query 'service:<service> @issue.id:<issue_id>' --limit 3` for `@error.stack`. Optional spread: `pup logs aggregate --query '@issue.id:<issue_id>' --compute count --group-by version` (which builds carry it). Keep it modest — a few samples, not bulk. Note in the writeup if no sample is retrievable (sampling/retention).
