...
Datadog access is the `pup` CLI (see the `fu-pup` skill in `fu-skills`) — not an MCP server. All log reads run through `pup` via Bash. **Auth preflight:** run `pup auth status`. If it reports not authenticated, or `expires_at` is inside this tick's likely runtime, and `has_refresh` is true → run `pup auth refresh` (non-interactive; mints a fresh ~60m token from the OS keychain). Only if refresh fails, or `has_refresh` is false, STOP and report: run `pup auth login` first. A `401` mid-sweep takes the same ladder — `pup auth refresh`, then `login` if that fails. A `403` is a missing scope, not an expired token: re-login with the needed scopes (refresh will not help). `pup` wraps responses as `{status,data,metadata}` — read the payload under `.data`.
