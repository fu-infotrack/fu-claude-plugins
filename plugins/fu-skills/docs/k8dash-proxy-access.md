# fu-k8 (formerly the fu-k8dash plugin) — read-only Kubernetes access via the k8dash proxy
...
- **Packaging:** a marketplace-plugin skill (`fu-k8` in `fu-skills`; originally the `fu-k8dash` plugin), not a per-project skill — the
  stage cluster hosts many projects, so the tool is reusable. Matches the
  `fu-pg` / `fu-mssql` naming and the config-resolved-host pattern.
