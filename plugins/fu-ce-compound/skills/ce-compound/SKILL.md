...
<!--
STANDALONE FORK NOTICE
This is a stripped, self-contained fork of EveryInc's compound-engineering
`ce-compound` skill, packaged to run alongside the superpowers plugin without
installing the full compound-engineering plugin.

Removed vs upstream (so it has zero external skill dependencies):
  - `ce-sessions` session-history pass (Phase 1 step 4 + the Full-mode follow-up
    question + the pre-resolved git-branch block that fed it).
  - `ce-compound-refresh` invocation (Phase 2.5). Replaced with a passive
    "Consolidation note" that surfaces overlap in the output but invokes nothing.

Kept (self-contained, bundled in this plugin):
  - references/schema.yaml, references/yaml-schema.md, assets/resolution-template.md,
    scripts/validate-frontmatter.py (stdlib-only python3).
  - Phase 3 specialized reviewers: ce-performance-oracle, ce-security-sentinel,
    ce-data-integrity-guardian, ce-code-simplicity-reviewer (bundled in agents/).

Upstream: https://github.com/EveryInc/compound-engineering-plugin
  plugins/compound-engineering/skills/ce-compound
To sync upstream fixes: re-fetch the upstream SKILL.md and re-apply the two
excisions above.
-->
...
Launch research subagents in parallel. Each returns text data to the orchestrator.

**Dispatch:** Launch `Context Analyzer`, `Solution Extractor`, and `Related Docs Finder` in parallel (background) and wait for all three to complete.
...
2. **Check the overlap assessment** from the Related Docs Finder before deciding what to write:
   ...
   | **Moderate** — same problem area but different angle, root cause, or solution | **Create the new doc** normally. Flag the overlap for the Consolidation Note below. |
...
After writing the new learning, surface any consolidation opportunity the Related Docs Finder uncovered — but **invoke no other skill**.

- If overlap was **High**, you already updated the existing doc (Phase 2 step 2); note which doc was updated.
- If overlap was **Moderate**, or the Related Docs Finder surfaced stale/contradicted candidate docs, add a one-line note to the output naming the overlapping/stale doc path(s) so the user can manually consolidate or refresh them later.
- If no related docs were found, or related docs still look consistent with the new learning, emit nothing.

In headless mode, communicate this via the `Consolidation note:` line of the terminal report rather than a prompt.

(Upstream `ce-compound` invokes the separate `/ce-compound-refresh` skill here; this standalone fork omits that skill and only surfaces a passive note.)

### Discoverability Check

After the learning is written and the consolidation note is decided, check whether the project's instruction files would lead an agent to discover and search `docs/solutions/` before starting work in a documented area. This runs every time — the knowledge store only compounds value when agents can find it.
...
- Any code-heavy issue → always run `ce-code-simplicity-reviewer` for minimal, clear examples.

These four agents are bundled with this plugin (see `agents/`). If a problem type does not match the first three, only `ce-code-simplicity-reviewer` runs (and only when the solution is code-heavy).
...
In lightweight mode, the overlap check is skipped (no Related Docs Finder subagent). This means lightweight mode may create a doc that overlaps with an existing one. That is acceptable. Only surface a Consolidation Note if there is an obvious narrow overlap target.
...
Consolidation note: <none | overlap with <path> | stale candidate: <path>>
...
**After displaying the interactive success output above, present the "What's next?" options using the platform's blocking question tool:** `AskUserQuestion` in Claude Code (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to numbered options in chat only when no blocking tool exists in the harness or the call errors — not because a schema load is required. Never silently skip the question. Do not continue the workflow or end the turn without the user's selection. (Interactive mode only — headless skips this per the headless block above.)
...
## Applicable Specialized Agents (bundled)

These agents ship with this plugin (`agents/`) and can review the documentation in Phase 3:
...
### When to Invoke
- **Auto-triggered** (optional): Agents can run post-documentation for enhancement (Phase 3, interactive Full mode only)
