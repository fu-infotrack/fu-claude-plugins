...
| fu-copilot | skill + agent + scripts | `/fu-copilot:copilot` + `@fu-copilot:copilot`. Delegate a task to GitHub Copilot CLI and verify what it actually did. Detached dispatch (runs take 10-113 min), `kill -0` wait, then git-based checks — because a self-report is not a completion signal |
...
| statusline | commands + scripts | Bash + jq status line renderer — drop-in for `npx ccstatusline` at 49x less time / 9x less memory per render; `/statusline-config` patches `settings.json` (statusLine is not a plugin capability) |
