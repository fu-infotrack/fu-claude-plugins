...
`fu-review-prs` ships a plain-bash suite for the posting-policy gate (COMMENT default,
`--auto-approve` opt-in, no flag leak across ticks). Hermetic — throwaway `$HOME` for the
state dir, `gh`/`git` stubbed on PATH, so it never posts to GitHub:
