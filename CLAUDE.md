...
- **Cosmetic-hook plugin** (`fu-herdr-tabs`): SessionStart + Stop hooks rename the caller's Herdr tab to the Claude session title. Herdr has no built-in for it — `tab` and `terminal_title_stripped` are separate sidebar tokens and nothing makes the label follow the agent. Three things generalise beyond this plugin. (1) **`$HERDR_PANE_ID`/`$HERDR_TAB_ID` are captured at pane start and go stale when a pane is re-homed** — measured: env said `w1F:t1`, `herdr tab rename "$HERDR_TAB_ID"` answered `tab_not_found`, and `herdr pane current --current` failed identically. Hence three-tier resolution — pane id, else the `session_id` off the hook payload, else sync every agent tab (each gets its own agent's title, so the fallback is lossless). (2) **`herdr` reports errors as JSON with exit status 0**, so `$?` proves nothing; every call checks for `.result`. `tab get` carries `label`, so no workspace/tab enumeration is needed — one read per tab about to be written. (3) **A SessionStart hook's stdout is injected into the session as context**, so the wrapper sends the engine's stdout to `/dev/null` and keeps diagnostics on stderr; the whole thing exits 0 on every path, because a tab label is cosmetic and failing a turn over one would not be. Config is `fu-tools` (`herdr-tabs.max_len`/`pin_prefix`/`scope`/`watch_interval`), each with an env override the test suite drives everything through; bad values fall back rather than reaching `--argjson` or `sleep`. The optional `start`/`stop` watcher is the only path that reaches background sessions attached into a pane, non-Claude agents, and `--bare` sessions — it is a **pidfile singleton** (`~/.claude` is shared by every session, so otherwise each new session would stack another), the pidfile is validated against `/proc/<pid>/cmdline` so a recycled pid is never killed, and the loop naps with `sleep & wait` so a TERM is acted on immediately instead of at the end of the interval. Glyph stripping is an **enumerated** list (`◐◑◒◓✳∙…`), not "leading punctuation", because `(wip) refactor` must survive — and herdr's own `terminal_title_stripped` is not enough (it strips `✳`, leaves `◐`).
...
`fu-herdr-tabs` ships a suite for the tab-label sync engine and its hook wrapper — the
three-tier caller resolution, glyph stripping, truncation, the pin opt-out, "never fail a
turn" on every broken-environment path, and the watcher's pidfile lifecycle. Hermetic:
`herdr` is stubbed on PATH (canned JSON in, a call log out), throwaway `$HOME` and state
dir, cwd moved out of the repo so `fu-config.sh` resolves empty:
...
- `herdr` — `fu-herdr-tabs` (plus `jq`). Errors come back as JSON with **exit status 0**, so `$?` proves nothing — check for `.result`.
...
| fu-herdr-tabs | hooks + command + scripts | Keep a Herdr tab's label in sync with the Claude session title — SessionStart/Stop hooks rename the calling pane's tab; `/herdr-tabs start` runs a background watcher for the panes and agents that never fire a Claude hook |
