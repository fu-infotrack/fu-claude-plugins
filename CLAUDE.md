...
mode, flag policy, the `/proc` cmdline confirmation, `BASELINE_HEAD` capture) and the
verification gates (`kill -0` wait with timeout, HEAD-moved, empty-commit naming and count,
the *vacuous* lossless case, dirty tree, log permission denials, and that a skipped check
never reads as a pass). Hermetic: `copilot` stubbed on PATH, throwaway `$HOME`, throwaway git
