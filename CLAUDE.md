...
- **Delegation agents are executors, not authors** (the `copilot` agent in `fu-skills`): the **caller** writes the task file and passes an absolute path; the agent only shells out to `copilot` and returns the reply verbatim. The first cut had the agent write the file from its own prompt, which put a **paraphrase hop** in the one place precision matters — the task text went caller prompt → sub-agent context → Sonnet re-types it → Copilot, so the "exact FINAL REPORT format" the file exists to pin down was the most likely thing to get reworded in transit. Caller-side authoring also removes the scratch-file hazard outright: **no repo git-ignores `.copilot-task.md`**, so a task file written to a repo root is untracked and a later `git add -A` commits it — the caller writes to `$CLAUDE_JOB_DIR/tmp` (background jobs) or `$(mktemp -d)` instead. `$CLAUDE_JOB_DIR` is **job-scoped, not universal** — absent in interactive sessions, and a sub-agent inherits only what its parent has, which is why the agent itself must never depend on it. What the sub-agent still earns is isolation of Copilot's output: it writes the reply to a caller-supplied path and returns a **4-line receipt** (`STATUS`/`OUTPUT`/`BYTES`/`VERDICT`) instead of the text. An earlier revision said "return the reply verbatim", which is an **unbounded** write into the calling session's context — a review of a large diff lands as thousands of tokens and dwarfs every input-side saving, and it is the one term you cannot size in advance. Same shape as et-sweep's investigator ("one-line receipt only") and fu-review-prs's decision sidecars. Note which side of the split is cheap: the task text passes through the caller's output once either way, so caller-vs-agent authoring is a ~100-token wash and was chosen for **fidelity**, not tokens — the return contract is where the context actually goes. `--allow-all-tools` is load-bearing (without it Copilot prompts for tool permission and **hangs** non-interactively), as is "do not solve it yourself" — a Sonnet agent holding Bash will otherwise just do the work and the delegation silently never happens.
...
- `copilot` (GitHub Copilot CLI, authenticated) — the `copilot` agent in `fu-skills`. Only that agent shells out to it; nothing else in the repo depends on it.
